...
def label_encode_ordinal(values: list, order: list) -> list:
    ...
    """
    values, order = np.array(values), np.array(order)

    mappingDict = {item: i for i, item in enumerate(order)}

    #print(mappingDict)

    encodings = np.zeros(len(values))

    for i in range(len(values)):
        if values[i] in mappingDict:
            encodings[i] = mappingDict[values[i]]
        else:
            encodings[i]=-1
